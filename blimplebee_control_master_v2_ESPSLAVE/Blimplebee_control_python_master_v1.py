...
class BlimpleBeeControl:
    ...
    def waypointcontrol(self, x, y, heading, xnew, ynew):
        """
        ...
        if headingtarget - heading > math.pi/4 or headingtarget - heading < -math.pi/4:
            # Spin on the spot
            # print("heading target :", math.degrees(headingtarget))
        ...
        # TODO: print only the location and heading of the blimp abd the target and make sure it is correct - looks like there might be issues here

    def heightcontrol2(self, height, targetheight):
        """
        ...
        at the targetheight.
        :param height: current heigt
        :param targetheight: target height
        """
        dadt = (height - self.oldheight) / (time.time() - self.oldtime)
        self.oldheight = height
        self.oldtime = time.time()

        targetspeed = self.pidheight.update_pid(targetheight, height)  # Decreases as we get closer to target - use the gains here to get a good negative slope on the target pwm as it gets closer to the target
    ...
    def heightcontrol(self, height, targetheight): # TODO: change to alt!!!
        """
        Height control function
        In this case the rate of change of altitude is calculated at each timestep and the aim is to reach zero
        at the targetheight.
        ...
        :param targetheight: target height
        """
        dadt = (self.oldheight - height) / (time.time() - self.oldtime)
        self.oldheight = height
        self.oldtime = time.time()

        targetaltspeed = self.pidheight.update_pid(targetheight, height)
        self.motorspeeda = self.motorspeeda + (dadt + targetaltspeed)    # TODO: change to self.motorspeeda so the loop actually works mate
        if self.motorspeeda > 200:
            self.motorspeeda = 200
        elif self.motorspeeda < -200:
            self.motorspeeda = -200

        print("Height is: ", height)
        print("Altspeedpwm is: ", self.motorspeeda)
...
class Run:
    ...
    def handledata(self, datapoint1, datapoint2, time):
        """
        Function to handle data and update the time and data records
        :param datapoint: data point to be recorded for this timestep
        :param time: timestep record
        :return:
        """
        if datapoint1 == "save" and datapoint2 == "plot":
            print(self.timerecord)
            print(self.datarecord1)
            print(self.datarecord2)
            plt.plot(self.timerecord, self.datarecord1, 'b')
            #plt.plot(self.timerecord, self.datarecord2, 'r', self.timerecord, self.datarecord1, 'b')
            plt.xlabel('x, pixels')
            plt.ylabel('y, pixels')
            plt.show()
        else:
            self.datarecord1.append(datapoint1)
            self.datarecord2.append(datapoint2)
            self.timerecord.append(time)
    ...
    def robotcontrol(self, x, y, heading, testnum):
        ...
        # Single height testing
            ...
            if (self.state % 2) == 0:
                print("going to 550")
                self.BlimpleBeeControl.heightcontrol2(x, 550)
                self.handledata(550, x, time.time())
                if (round(time.time() * 1000)) - self.timenow > int:
                    self.state = self.state + 1
                    self.timenow = (round(time.time() * 1000))
            else:
                print("going to 290")
                self.BlimpleBeeControl.heightcontrol2(x, 290)
                self.handledata(290, x, time.time())
                if (round(time.time() * 1000)) - self.timenow > int:
                    self.state = self.state + 1
                    self.timenow = (round(time.time() * 1000))
            ...
            if (round(time.time()) * 1000) - self.timenow > int:
                ...
                # TODO: add in a state change to move between waypoints
    ...
    def randommovementtest(self):
        ...
        """
        if (round(time.time() * 1000) - self.timenow) > straightint:
            self.BlimpleBeeControl.motorcontrol("left", 1, pwm)
            self.BlimpleBeeControl.motorcontrol("right", 0, pwm)
            self.timenow = round(time.time() * 1000)
            self.i = 1
            print("turn")
        if self.i == 1:
            if (round(time.time() * 1000) - self.timenow) > turnint:
                self.i = 0
                self.timenow = round(time.time() * 1000)
        """
    ...
    def runwithvision(self):
        ...
        while True:
            ...
            if not len(corners) == 0 and ids[0] == 1:
                ...
                # Draw detected markers: TODO: check this still displays correctly
